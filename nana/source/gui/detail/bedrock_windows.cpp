...
#pragma pack()
	...
	std::wstring class_name_wstr;

	LPCWSTR windows_class_name()
	{
		std::stringstream ss;

		if (class_name == nullptr)
		{			
			ss << "NW" << reinterpret_cast<int>(&windows_class_name);
			std::wstring_convert<std::codecvt_utf8<wchar_t>> myconv;
			auto s = myconv.from_bytes(ss.str());
			class_name_wstr = myconv.from_bytes(ss.str());
			class_name = class_name_wstr.c_str();
		}
			...
			impl_(new private_impl)
	{
		...
		wincl.lpszClassName = windows_class_name(); //L"NanaWindowInternal"; 
	...
	bedrock::~bedrock()
	{
		...
		delete impl_;
		delete pi_data_;
		::UnregisterClass(/*L"NanaWindowInternal"*/ windows_class_name(), windows_module_handle());
	}
